...
class Level:
    ...
    def getLevel(self) -> dict:
        ...
        levelDict = {
            "VERSION" : float(raw[0]),
            "LEVEL DIMENSIONS" : [float(raw[3]),float(raw[4])],
            "TOOL DATA" : toolData,
            "QUICK SLOTS" : slots,
    ...
    def addObject(self, _type, data):
        ...
        objects = self.localData["PLACED OBJECTS"]
        sameTypeObjects = objects[_type]
        quantity = sameTypeObjects["QUANTITY"]
        sameTypeObjects["QUANTITY"] += 1
        sameTypeObjects[_type + str(quantity)] = data
        objects[_type] = sameTypeObjects
        self.localData["PLACED OBJECTS"] = objects

    def connect(self, obj0, obj1):
        wires = self.localData["WIRES"]
        if wires["WIRES"].__contains__([obj1, obj0]):
            return
        wires["QUANTITY"] += 1
        wires["WIRES"].append([obj0, obj1])
        self.localData["WIRES"] = wires